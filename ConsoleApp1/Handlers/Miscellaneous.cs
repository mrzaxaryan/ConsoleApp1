...
public static unsafe class Miscellaneous
{
    public static bool Handle(byte opcode, CONTEXT* ctx, byte* address, Action<string, int> Log)
    {
        switch (opcode)
        {
            ...
            case 0x39:
                return HandleCmpEvGv32(ctx, address, Log);

            case 0x88:
                return HandleMov_Rm8_R8(ctx, address, Log);   // MOV r/m8,r8

            case 0x8A:
                return HandleMov_R8_Rm8(ctx, address, Log);   // MOV r8,r/m8

            case 0x38:
                return HandleCmpEvGv8(ctx, address, Log);     // CMP r/m8, r8

            case 0x3A:
                return HandleCmpGvEv8(ctx, address, Log);     // CMP r8, r/m8
            default:
                return false;
        }
    }
    ...
    private static unsafe bool HandleCmpEvGv8(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
    ...
    private static unsafe bool HandleCmpGvEv8(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
    ...
    private static unsafe bool HandleMov_Rm8_R8(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
        // 88 /r → MOV r/m8, r8
        int offs = 1;
        byte modrm = ip[offs++];
    ...
    private static unsafe bool HandleMov_R8_Rm8(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
        // 8A /r → MOV r8, r/m8
        int offs = 1;
        byte modrm = ip[offs++];
    ...
    private static unsafe bool HandleCmpEvGv32(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
    ...
    private static unsafe uint GetReg32_NoRex(CONTEXT* c, int r) => (r & 7) switch
    {
    ...
    private static unsafe ulong GetReg64_NoRex(CONTEXT* c, int r) => (r & 7) switch
    {
        0 => c->Rax,
        1 => c->Rcx,
        2 => c->Rdx,
    ...
    private static string Reg64Name_NoRex(int r) => (r & 7) switch
    {
        0 => "RAX",
        1 => "RCX",
        2 => "RDX",
    ...
    private static string Reg32Name(int r) => (r & 7) switch
    {
        0 => "EAX",
        1 => "ECX",
        2 => "EDX",
    ...
    private static uint UpdateFlags_Sub32(uint oldFlags, uint a, uint b, uint r)
    {
