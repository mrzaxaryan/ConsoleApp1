...
public static unsafe class MovOperations
{
    public static bool Handle(byte opcode, CONTEXT* ctx, byte* address, Action<string, int> Log)
    {
        switch (opcode)
        {
            ...
            case >= X64Opcodes.MOV_RAX_IMM64 and <= X64Opcodes.MOV_RDI_IMM64:
                return HandleMovImmToReg(ctx, address, Log);
    ...
    private static unsafe bool HandleMovRm32Imm32(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
    ...
    private static unsafe bool HandleMovRm64R64(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
    ...
    private static unsafe bool HandleMovImmToReg(CONTEXT* ctx, byte* address, Action<string, int> Log)
    {
    ...
    private static unsafe bool HandleMovR32Rm32(CONTEXT* ctx, byte* ip, Action<string, int> Log)
    {
        // 8B /r → MOV r32, r/m32
        int offs = 0;
